...
public class Board : MonoBehaviour
{
    ...
    private void Setup()
    {
        for (int x = 0; x < width; x++)
        {
            for (int y = 0; y < height; y++)
            {
                ...
                int iterations = 0;
                while (MatchseAt(new Vector2Int(x, y), gems[gemToUse]) && iterations < 100)
                {
    ...
    bool MatchseAt(Vector2Int posToCheck, Gem gemToCheck)
    {
        if (posToCheck.x > 1)
        {
    ...
    private void CheckMisplacedGames()
    {
    ...
    public void ShuffleBoard()
    {
        if (currentState != BoardState.wait)
        {
            ...
            List<Gem> gemsForBoard = new List<Gem>();

            for(int x = 0; x < width; x++)
            {
                for (int y = 0; y < height; y++)
                {
                    gemsForBoard.Add(allGems[x, y]);
                    allGems[x, y] = null;
                }
            }
            ...
            for (int x = 0; x < width; x++)
            {
                for (int y = 0; y < height; y++)
                {
                    ...
                    int gemToUse = Random.Range(0, gemsForBoard.Count);

                    int iteration = 0;
                    while (MatchseAt(new Vector2Int(x, y), gemsForBoard[gemToUse])&& iteration <100 && gemsForBoard.Count >1)
                    {
                        gemToUse = Random.Range(0, gemsForBoard.Count);
                        iteration++;
                    }

                    gemsForBoard[gemToUse].Setupgem(new Vector2Int(x, y), this);
                    allGems[x, y] = gemsForBoard[gemToUse];
                    gemsForBoard.RemoveAt(gemToUse);
                }
            }
